...
import { getContratos, deleteContrato, getHistorico, addHistorico, updateHistorico, deleteHistorico } from '../api/copasaService';
...
const CopasaPage = () => {
  ...
  const handleGenerateReport = () => {
    if(!selectedContrato) return;
    const reportUrl = `http://localhost:3001/api/copasa/${selectedContrato.id}/report`;
  ...
  const filteredContratos = useMemo(() => {
    ...
    return contratos.filter(contrato => 
        ...
        (contrato.matricula_copasa?.toLowerCase().includes(lowerCaseSearchTerm)) ||
        ...
        (contrato.identificador_usuario?.toLowerCase().includes(lowerCaseSearchTerm)) ||
        (contrato.centralizadora?.toLowerCase().includes(lowerCaseSearchTerm))
    );
  ...
  return (
    <div>
      ...
      <Modal isOpen={isViewModalOpen} onClose={handleCloseModals} title={`Detalhes: ${selectedContrato?.endereco}`}>
          ...
          {loadingHistory ? <p>Carregando histórico...</p> : <HistoryTable history={historyData} type="copasa" onEdit={handleEditHistoryClick} onDelete={handleDeleteHistoryClick} />}
      </Modal>
      ...
      <Modal isOpen={isHistoryFormModalOpen} onClose={handleCloseModals} title={selectedHistory?.id ? 'Editar Fatura' : 'Adicionar Fatura'}>
        <HistoryForm initialData={selectedHistory} onSave={handleSaveHistory} type="copasa" />
      </Modal>
